...
main_frame = Frame(root,bg="Beige")
main_frame.pack(fill=BOTH, expand=True)

"""homepage"""

homepage = Frame(main_frame,bg="Beige")
...
titlelbmain = customtkinter.CTkLabel(homepage,
                text="Ulti-mateXO",
                font=my_font,
                text_color="#F28500",
                justify="center")
...
chose_frame = Frame(main_frame,bg="Beige")

#ข้อความ
my_font1 = customtkinter.CTkFont(family="Helvetica",
            size=60,
            weight="bold",
            slant="roman",
            underline=True,)

heading = customtkinter.CTkLabel(chose_frame, text="Choose Your Mode",text_color="#F28500",
                               font=my_font1)
heading.pack(pady=50)

#รูปแบบปุ่ม
ButtonType = Frame(chose_frame, bg="Beige")
...
game1 = Frame(main_frame,bg="Beige")
...
b1 = Button(game1, text=" ", font=("Helvetica",20),height=4,width=8,bg="#4169E1",fg="white", command= lambda: b_click_general(b1))
b2 = Button(game1, text=" ", font=("Helvetica",20),height=4,width=8,bg="#4169E1",fg="white", command= lambda: b_click_general(b2))
b3 = Button(game1, text=" ", font=("Helvetica",20),height=4,width=8,bg="#4169E1",fg="white", command= lambda: b_click_general(b3))

b4 = Button(game1, text=" ", font=("Helvetica",20),height=4,width=8,bg="#4169E1",fg="white", command= lambda: b_click_general(b4))
b5 = Button(game1, text=" ", font=("Helvetica",20),height=4,width=8,bg="#4169E1",fg="white", command= lambda: b_click_general(b5))
b6 = Button(game1, text=" ", font=("Helvetica",20),height=4,width=8,bg="#4169E1",fg="white", command= lambda: b_click_general(b6))

b7 = Button(game1, text=" ", font=("Helvetica",20),height=4,width=8,bg="#4169E1",fg="white", command= lambda: b_click_general(b7))
b8 = Button(game1, text=" ", font=("Helvetica",20),height=4,width=8,bg="#4169E1",fg="white", command= lambda: b_click_general(b8))
b9 = Button(game1, text=" ", font=("Helvetica",20),height=4,width=8,bg="#4169E1",fg="white", command= lambda: b_click_general(b9))
...
def on_click(row, col):
    ...
    if selected_size and (board[row][col] is None or (sizes[selected_size] > sizes[board[row][col][1]] and board[row][col][0] != player_advand)) and not check_winner():
        if pieces[player_advand][selected_size] > 0:
            ...
            buttons[row][col].config(text=f"{player_advand} ({selected_size})",fg=colors[player_advand])  # อัปเดตปุ่ม
...
def reset_game():
    ...
    for row in range(3):
        for col in range(3):
            buttons[row][col].config(text=" ",bg="#FF6F61")  # ล้างข้อความในปุ่ม
    update_pieces_label()  # อัปเดตป้ายจำนวนชิ้นส่วน
...
game_frame = Frame(main_frame,bg="Beige")  # สร้างเฟรมสำหรับเกม
...
for row in range(3):
    for col in range(3):
        button = Button(game_frame, text="", width=19, height=7,bg="#FF6F61", fg=colors["text"], command=lambda r=row, c=col: on_click(r, c))  # ปุ่มสำหรับวางชิ้นส่วน
...
for size in sizes:
    size_buttons[size] = customtkinter.CTkButton(game_frame, text=size.capitalize(),
                                command=lambda s=size: select_size(s), # ปุ่มเลือกขนาด
                                width=120,
                                height=40,
                                corner_radius=10,
                                fg_color="#BE2623",
...
replay_button = customtkinter.CTkButton(game_frame, text="Replay",
                        command=reset_game,  # ปุ่มรีเพลย์
                        width=120,
                        height=40,
                        corner_radius=10,
                        fg_color="#0080FE",
...
turn_label = Label(game_frame, text=f"Player: {player_advand}",bg="Beige",fg="#F28500",font=("Helvetica",15, "bold"))  # ป้ายเพื่อแสดงผู้เล่นปัจจุบัน
turn_label.place(relx=0.5,rely=0.85,anchor=CENTER)  # วางมันไว้ด้านล่างบอร์ดเกม

pieces_label = Label(game_frame, text=f"X Pieces: {pieces['X']}",bg="Beige",fg="#F28500",font=("Helvetica",15, "bold"))  # ป้ายเพื่อแสดงจำนวนชิ้นส่วนที่เหลือ
pieces_label.place(relx=0.5,rely=0.9,anchor=CENTER)  # วางป้ายไว้ด้านล่าง

pieces_label1 = Label(game_frame, text=f"O Pieces: {pieces['O']}",bg="Beige",fg="#F28500",font=("Helvetica",15, "bold"))  # ป้ายเพื่อแสดงจำนวนชิ้นส่วนที่เหลือ
...
#ปุ่มของhomepage
bpm = customtkinter.CTkButton(homepage,
            text="Play",
...
general = customtkinter.CTkButton(ButtonType,
                text="General Mode",
...
advance = customtkinter.CTkButton(ButtonType,
                text="Advance Mode",
                font=("Helvetica", 25, "bold"),
                height=80,
                width=80,
...
back = customtkinter.CTkButton(ButtonType,
            text="Back",
            font=("Helvetica", 25, "bold"),
            height=80,
            width=80,
...
back_bt_game1 = customtkinter.CTkButton(game1, text="Back",
                    ...
                    border_color="#F28500",
                    state="normal",
                    command=move_page_chose)
back_bt_game1.grid(row=3, column=1, pady=10)

#ปุ่มกลับไปหน้าเลือกโหมดของgamemodeadvand
back_bt_game2 = customtkinter.CTkButton(game_frame, text="Back",
                    width=120,
                    height=40,
                    corner_radius=10,
                    fg_color="#0080FE",
